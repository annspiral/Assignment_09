...
# AAllen, 2020-Sept-08, added tests for ScreenIO: get_CD_ID_choice and
#                       get_Track_ID_choice
# AAllen, 2020-Sept-08, added tests for Processing Class: remove_track, 
#                       find_cd_id and find_track_id
#-------------------------------------------#
...
print('Inventory after adding CD:')
for item in lstOfCDObjects:
    print(item)
print('CD ID selection: {}'.format(IO.ScreenIO.get_CD_ID_choice(lstOfCDObjects)))
cd_idx = 1
...
print('CD ID selection: {}'.format(IO.ScreenIO.get_Track_ID_choice(cd)))
...
for item in lstOfCDObjects:
    print(item)
print('Add a track:')
PC.DataProcessor.add_track((2, 'test.trackTWO', '02:59'), lstOfCDObjects[0])
for item in lstOfCDObjects[0].cd_tracks:
    print(item.get_record())
print('Remove a track:')
PC.DataProcessor.remove_track(2, lstOfCDObjects[0])
for item in lstOfCDObjects[0].cd_tracks:
    print(item.get_record())
if PC.DataProcessor.find_cd_id(lstOfCDObjects,1):
    print('CD ID Found')
if PC.DataProcessor.find_track_id(lstOfCDObjects[0],1):
    print('CD Track ID Found')
